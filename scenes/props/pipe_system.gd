...
static func get_openings(item: int, rotation: int) -> Array:
	var base_op := get_base_openings(item)
	var result := []
	for d in base_op:
		result.append(rotate_dir(d, rotation))
	return result
...
func place_pipe(gx: int, gz: int, item: int, rotation: int) -> bool:
	return _place_internal(gx, gz, item, rotation, false)

func place_pump(gx: int, gz: int, rotation: int) -> bool:
	return _place_internal(gx, gz, BuildItem.PUMP, rotation, true)

func _place_internal(gx: int, gz: int, item: int, rotation: int, is_pump: bool) -> bool:
	if is_pump:
	...
	if pipes.has(key):
		return false
	var node := _create_pipe_node(gx, gz, item, rotation)
	if is_pump:
		node["is_pump"] = true
	pipes[key] = node
...
func update_pipe_rotation(gx: int, gz: int, new_rotation: int) -> void:
	var key := Vector2i(gx, gz)
	if not pipes.has(key):
		return
	var node: Dictionary = pipes[key]
...
func _create_pipe_node(gx: int, gz: int, item: int, rotation: int) -> Dictionary:
...
func _refresh_visual(key: Vector2i) -> void:
	...
	var rotation: int = node["rotation"]
	var flowing: bool = node["flowing"]
	var is_pump: bool = node.get("is_pump", false)
	visual.rotation_degrees.y = -(rotation * 90.0 + PIPE_ROTATION_OFFSETS[node["item"]])
	if not is_pump:
...
func _process(delta: float) -> void:
	...
	if _green_spread:
		_green_spread.tick(delta, _world_gen.container_levels)
