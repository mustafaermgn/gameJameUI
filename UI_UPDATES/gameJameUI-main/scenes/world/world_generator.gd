...
func _ready() -> void:
	...
	if world_seed == 0:
		...
		rng.seed = world_seed
	_init_biome_noise(rng.seed)
	_generate_biome_map()
	_create_ground()
	_reserve_spawn_area()
	_generate_obstacles(rng)
	_generate_water_pools(rng)
	_generate_containers(rng)
	_generate_biome_scatter(rng)
...
func _generate_water_pools(rng: RandomNumberGenerator) -> void:
	for i in range(WATER_POOL_COUNT):
		...
		for _attempt in range(50):
			...
			var pool_set := {Vector2i(gx, gz): true}
