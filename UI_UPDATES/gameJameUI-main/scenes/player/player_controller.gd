...
func _face_direction() -> void:
	...
	var target_quat := target_transform.basis.orthonormalized().get_rotation_quaternion()
	model.quaternion = model.quaternion.slerp(target_quat, 0.15)
	model.scale = Vector3.ONE * (MODEL_HEIGHT / 1.7) # Ensure scale is preserved
